...
<div style="font-family:'Segoe UI',Arial,sans-serif;max-width:580px;color:#1e2849">
  ...
  async getListElements(listId) {
    const res = await fetch(`/api/content/list?id=${listId}`, { credentials: 'same-origin' });
    const data = await res.json();
    if (!data.ok) throw new Error(data.error || 'Не удалось загрузить список');
  ...
  async getDiskFileLink(fileId) {
    const res = await fetch(`/api/content/file?id=${fileId}`, { credentials: 'same-origin' });
    const data = await res.json();
