...
const LIST_IDS = {
  packages:        123,  // «Пакеты консалтинга — Акселератор × Сколково»
  materials:       125,  // «Материалы — Акселератор × Сколково»
  services:        127,  // «Услуги — Акселератор × Сколково»
  settings:        129,  // «Настройки ЛК — Акселератор × Сколково»
  vendors:         0,    // «Вендоры — Аутентификация» (управляется бэкендом)
  partnerPackages: 143   // «Партнёрская программа — Акселератор × Сколково»
};
...
const MOCK_PACKAGES = [
  {
    ...
    items: [
      ...
      { icon: 'ti-user-check', text: 'Онбординг-встреча с менеджером Акселератора (до 1 ч.)' },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: 'ti-users',          text: '2-3 рабочих сессии с экспертами Акселератора' },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: 'ti-users',        text: '5+ рабочих сессий с выделенной командой Акселератора' },
...
const MOCK_SERVICES = [
  ...
  { id:'svc-3',  name:'Онбординг-встреча с Акселератором (до 1 ч.)',
  ...
  { id:'svc-6',  name:'Анализ конкурентов в партнёрском канале',
    ...
    price:0, priceText:'По запросу', standalone:true,
    description:'Кто уже в канале Акселератора, на каких условиях, и как ваш продукт дифференцируется' },
  ...
  { id:'svc-9',  name:'Рабочие сессии с экспертами Акселератора (2–3)',
    icon:'ti-users',          pkg:'audit', pkgLabel:'Аудит',
    price:0, priceText:'По запросу', standalone:true,
    description:'Структурированные рабочие сессии по результатам аудита с командой Акселератора' },
  ...
  { id:'svc-17', name:'Рабочие сессии с командой Акселератора (5+)',
...
const Content = {
  ...
  async getMaterials() {
    return this._withCache('content_materials_v3', async () => {
  ...
  async getSettings() {
    if (!LIST_IDS.settings) return {};
    try {
      return await this._withCache('content_settings', async () => {
  ...
  clearCache() {
    ['content_packages', 'content_materials', 'content_materials_v2', 'content_materials_v3',
     'content_services', 'content_settings', 'content_partner'].forEach(k => localStorage.removeItem(k));
  },
